...
class Env():
    ...
    def get_bs_delta(self):
        d1, _ = option_functions._d(self.option['S/K']*self.K, self.K, self.r, self.q, self.v, self.option['T']/self.days_in_year)
        return norm.cdf(d1)
    ...
    def __update_option(self):
        ...
        iv = calc_impl_volatility(spot, self.K, self.r, self.q, ttm/self.days_in_year, P)
            ...
            if self.process == 'Real':
                ...
                while new_set is None:
                    ...
                    if self.process == 'GBM':
                        ...
                        # --- FIX START ---
                        # 1. Determine the dates we will use
                        # start_idx = self.r_df[self.r_df['Date'] == quote_datetime].index[0]
                        # end_idx = start_idx + self.steps # e.g. 35 steps
                        
                        # # Check bounds
                        # if end_idx >= len(self.r_df):
                        #     continue
                            
                        # end_date_str = self.r_df.iloc[end_idx]['Date']
                        
                        # # 2. Calculate Real Calendar T (accounting for weekends)
                        # d1 = datetime.strptime(quote_datetime, '%Y-%m-%d')
                        # d2 = datetime.strptime(end_date_str, '%Y-%m-%d')
                        # real_T = (d2 - d1).days
                        
                        # # 3. Simulate using the Real Time span
                        # self.sim.simulate(1.0, real_T, self.steps)
                        # --- FIX END ---
                    ...
                    self.sim.simulate(1.0, T, dt)
                    new_set = self.sim.return_set(.85, 1.15, quote_datetime, 40, 90, sorted(self.r_df['Date'].unique()), self.r)
